<?php
include 'config.php'; // Amit Sonkusale
session_start();
error_reporting(0);

// On/Off state
// AC
$sql_onoff_ac = "SELECT `c3state` FROM `amit_ka_table` WHERE `equipments` = 'Air Conditioner'";
...
// Ref
$sql_onoff_ref = "SELECT `c3state` FROM `amit_ka_table` WHERE `equipments` = 'Refrigerator'";
...
// Fan
$sql_onoff_fan = "SELECT `c3state` FROM `amit_ka_table` WHERE `equipments` = 'Fan'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['acon'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=1 WHERE `equipments` = 'Air Conditioner'";
        $result = mysqli_query($conn, $sql);
    }

    if (isset($_POST['acoff'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=0 WHERE `equipments` = 'Air Conditioner'";
        $result = mysqli_query($conn, $sql);
    }
    ...
    if (isset($_POST['refon'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=1 WHERE `equipments` = 'Refrigerator'";
        $result = mysqli_query($conn, $sql);
    }

    if (isset($_POST['refoff'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=0 WHERE `equipments` = 'Refrigerator'";
        $result = mysqli_query($conn, $sql);
    }
    ...
    if (isset($_POST['fanon'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=1 WHERE `equipments` = 'Fan'";
        $result = mysqli_query($conn, $sql);
    }

    if (isset($_POST['fanoff'])) {
        $sql = "UPDATE `amit_ka_table` SET `c3state`=0 WHERE `equipments` = 'Fan'";
        $result = mysqli_query($conn, $sql);
    }
}
