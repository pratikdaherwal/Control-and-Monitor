...
// AC ON
$sql_ac_on_submit = "SELECT `c3on_time` FROM `timeout` WHERE `equipments` = 'air conditioner'";
...
if ($result_ac_on_submit->num_rows > 0) {
    ...
    while ($row = $result_ac_on_submit->fetch_assoc()) {
        $setacon = $row['c3on_time'];
    }
}
// AC OFF
$sql_ac_off_submit = "SELECT `c3off_time` FROM `timeout` WHERE `equipments` = 'air conditioner'";
...
if ($result_ac_off_submit->num_rows > 0) {
    ...
    while ($row = $result_ac_off_submit->fetch_assoc()) {
        $setacoff = $row['c3off_time'];
    }
}
// REF ON
$sql_ref_on_submit = "SELECT `c3on_time` FROM `timeout` WHERE `equipments` = 'refigerator'";
...
if ($result_ref_on_submit->num_rows > 0) {
    ...
    while ($row = $result_ref_on_submit->fetch_assoc()) {
        $setrefon = $row['c3on_time'];
    }
}
// REF OFF
$sql_ref_off_submit = "SELECT `c3off_time` FROM `timeout` WHERE `equipments` = 'refigerator'";
...
if ($result_ref_off_submit->num_rows > 0) {
    ...
    while ($row = $result_ref_off_submit->fetch_assoc()) {
        $setrefoff = $row['c3off_time'];
    }
}

// FAN ON
$sql_fan_on_submit = "SELECT `c3on_time` FROM `timeout` WHERE `equipments` = 'fan'";
...
if ($result_fan_on_submit->num_rows > 0) {
    ...
    while ($row = $result_fan_on_submit->fetch_assoc()) {
        $setfanon = $row['c3on_time'];
    }
}
// FAN OFF
$sql_fan_off_submit = "SELECT `c3off_time` FROM `timeout` WHERE `equipments` = 'fan'";
...
if (isset($_POST['acsubmit'])) {
    $sql = "UPDATE `timeout` SET `c3on_time`= $setacon, `c3off_time` = $setacoff WHERE `equipments` = 'air conditioner'";
    $result = mysqli_query($conn, $sql);
}
// ref submit button
if (isset($_POST['refsubmit'])) {
    $sql = "UPDATE `timeout` SET `c3on_time`= $setrefon, `c3off_time` = $setrefoff WHERE `equipments` = 'refrigerator'";
    $result = mysqli_query($conn, $sql);
}
// fan submit button
if (isset($_POST['fansubmit'])) {
    $sql = "UPDATE `timeout` SET `c3on_time`= $setfanon, `c3off_time` = $setfanoff WHERE `equipments` = 'fan'";
    $result = mysqli_query($conn, $sql);
}
...
<body>
    ...
    <span class="highlight" style="text-align: center;"> Chamber 3</span> </div>
    <div style=height:30px;></div>
